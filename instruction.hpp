...
class Instruction {
    public:
        int PC;                 // Base 10 (from hexadecimal) value representing the instruction address.
        iType type;             // The type of the instruction.
        vector<int> dependents; // List of PC values of instructions that the current instruction 
                                // is dependent on.
        ...
        Instruction(int PC, iType type, vector<int> dependents): PC(PC), type(type), dependents(dependents) {};
};